...
TabProbsResults Bos::SEstep(const mat& V, const mat& W)
{
	cube cubeProbs = this->getCubeProbs();
...
mat Bos::SEstepRowRandomParamsInit(mat& Wsample, uvec& colSample){
	mat result(_N, _kr);
	result.zeros();
	...
	for (size_t d = 0; d < Wsample.n_rows; d++)
	{
		for (size_t h = 0; h < _kc; h++)
		{
			if(Wsample(d, h)==1){
				for (size_t i = 0; i < _N; i++)
				{
					...
					for (size_t k = 0; k < _kr; k++)
					{
						double accum = _cubeProbs(k, h, xsample(i, d) - 1);
						result(i, k) +=  log(accum);	
					}
				}
			}
...
void Bos::imputeMissingData(const mat& V, const mat& W) {
...
void Bos::MstepVW(const mat& V, const mat& W, bool init)
{
	for (int k = 0; k < _kr; k++)
	{
		for (int h = 0; h < _kc; h++) {
...
void Bos::MstepInitRandomParams(mat xsample, mat Vsample, mat Wsample){
	for (int k = 0; k < _kr; k++)
	{
		for (int h = 0; h < _kc; h++) {
			mat datablock_kh;
			...
			datablock_kh = xsample.submat(rowind, colind);
			colvec datablock_kh_col = vectorise(datablock_kh, 0);
			colvec tabmu0 = linspace<colvec>(1, _m, _m);
			colvec tabp0;
...
cube Bos::gettabpej() {
	...
	if (_m == 7) {
		...
		result.slice(3) << 319847 / 12700800 << (double)-50509 / 2540160 << (double)104207 / 6350400 << (double)991 / 141120 << (double)-289 / 15120 << (double)-5293 / 105840 << (double)-71 / 882 << endr
	...
	if (_m == 8) {
		...
		result.slice(1) << 46847 / 80640 << (double)61 / 2304 << (double)-7363 / 80640 << (double)-857 / 5376 << (double)-183 / 896 << (double)-151 / 640 << (double)-83 / 320 << (double)-621 / 2240 << endr
